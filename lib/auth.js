...
/**
 * Holt das Session-Token aus dem Request.
 *   - Web:    httpOnly-Cookie (wird vom Browser automatisch mitgeschickt)
 *   - Native: Authorization: Bearer <token> (iOS/Android-WebView; dort sind
 *             Third-Party-Cookies unzuverlässig bzw. blockiert)
 */
...
function setSessionCookie(res, token) {
  ...
  res.cookie(COOKIE_NAME, token, {
    httpOnly: true,
    secure: isProd,
    // In Produktion 'none', damit der Cookie auch aus der nativen App
    // (anderer Origin) gesendet werden darf. Erfordert secure: true.
...
async function registerEmail({ email, password, displayName, profile }) {
  ...
  const { data, error } = await supabase
    ...
    .insert({
      ...
      email,
      password_hash,
      display_name: String(displayName || '').trim().slice(0, 60) || null,
      profile: profile || {},
    })
...
/** Namen nachträglich setzen/ändern (z.B. für E-Mail-Schüler über das Menü) */
async function updateDisplayName(studentId, displayName) {
  const name = String(displayName || '').trim().slice(0, 60);
  if (!name) throw httpError(400, 'Bitte einen Namen eingeben.');
  const { data, error } = await supabase
    .from('students')
    ...
    .select('id, auth_method, email, display_name, class_id, profile')
    .single();
  if (error) throw httpError(500, 'Name konnte nicht gespeichert werden.');
  return data;
}
