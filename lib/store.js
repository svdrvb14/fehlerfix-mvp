...
/**
 * Löscht AUSSCHLIESSLICH Testdaten mit dem Marker "ClaudeQA".
 * Sicherheit: matcht nur exakt diese Präfixe, kann keine echten Nutzer treffen.
 * Reihenfolge: Schüler (cascade state+log) → Klassen → Lehrer.
 */
async function deleteClaudeQATestData() {
  const counts = {};
  const stud = await supabase
    .from('students')
    .delete({ count: 'exact' })
  ...
  const teach = await supabase
    .from('teachers')
    .delete({ count: 'exact' })
    .ilike('email', 'claudeqa.%@fehlerfix.test');
