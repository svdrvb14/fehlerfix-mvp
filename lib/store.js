...
function rowToState(row, profile) {
  return {
    ...
    wordRegister: row?.word_register || [],   // Merkwörter (mehrfach falsch geschrieben)
    recentTypes: row?.recent_types || [],     // zuletzt genutzte Übungsformate (Abwechslung)
    coins: row?.coins ?? 0,                   // Insel-Währung (getrennt von points/Level)
    islandItems: row?.island_items || [],     // gekaufte Insel-Gegenstände
  };
}
...
async function saveStudentState(studentId, state) {
  await supabase
    ...
    .update({
      ...
      word_register: state.wordRegister || [],
      recent_types: (state.recentTypes || []).slice(0, 8),
      coins: state.coins ?? 0,
      island_items: state.islandItems || [],
...
async function getStudentDetail(studentId) {
  ...
  return {
    ...
    wordRegister: require('./wordregister').activeWords(st?.word_register),
    recentExercises: log || [],
  };
}
...
/**
 * Bestenliste innerhalb einer Klasse.
 * range 'week' = Übungen der letzten 7 Tage (aus exercise_log),
 * range 'all'  = Gesamtpunkte aus student_state.
 */
async function getClassLeaderboard(classId, range = 'week') {
  if (!classId) return [];
  const { data: students } = await supabase
    .from('students')
  ...
  if (range === 'all') {
    ...
    for (const s of list) {
      const { data: st } = await supabase
        .from('student_state')
        .select('points, level, exercises_completed')
        .eq('student_id', s.id)
        .maybeSingle();
      out.push({
        id: s.id,
        name: s.display_name || s.email || 'Schüler/in',
        score: st?.points || 0,
        detail: `Level ${st?.level ?? 1} · ${st?.exercises_completed ?? 0} Übungen`,
      });
    }
    return out.sort((a, b) => b.score - a.score);
  }

  // Woche: Anzahl Übungen der letzten 7 Tage
  ...
  for (const s of list) {
    ...
    out.push({
      ...
      name: s.display_name || s.email || 'Schüler/in',
      score: count || 0,
      detail: (count || 0) === 1 ? 'Übung diese Woche' : 'Übungen diese Woche',
    });
  }
  return out.sort((a, b) => b.score - a.score);
}
